...
class Guzzle5HttpHandlerTest extends BaseTest
{
    ...
    public function setUp()
    {
        ...
        $this->mockClient = $this->getMockBuilder('GuzzleHttp\Client')
            ...
            ->getMock();
        $this->mockFuture = $this->getMockBuilder('GuzzleHttp\Ring\Future\FutureInterface')
            ->disableOriginalConstructor()
            ->getMock();
    }
    ...
    public function testAsyncWithoutGuzzlePromiseThrowsException()
    {
        ...
        foreach (spl_autoload_functions() as $function) {
            if ($function[0] instanceof ClassLoader) {
                ...
                spl_autoload_register($newAutoloadFunc = [
                    $newAutoloader,
                    'loadClass'
                ]);
    ...
    public function testSuccessfullySendsRequestAsync()
    {
        ...
        $this->mockClient->expects($this->any())
            ->method('send')
            ->will($this->returnValue(new FutureResponse(new CompletedFutureValue($response))));
        $this->mockClient->expects($this->any())
            ->method('createRequest')
            ->will($this->returnValue($this->mockRequest));
    ...
    public function testPromiseHandlesException()
    {
        $this->mockClient->expects($this->any())
            ->method('send')
            ->will($this->returnValue(new FutureResponse((new CompletedFutureValue(new Response(200)))->then(function () {
